...
def pathfinder(name, opt_1, opt_2, opt_3):
    ...
    #print(demand.head())
    #print(demand.index.get_level_values(2).isin(['b']))
    ...
    for target in targets:
        ...
        #prblToGo.solve(pulp.apis.GLPK_CMD(fracGap=0.01, maxSeconds = 60))
        #prblToGo.solve(pulp.apis.PULP_CBC_CMD(gapRel=0.01, timeLimit=60 ))
        #prblToGo.solve()
        #schreibe Status
        ...
        #test bzgl. Schattenpreisen

        o = [{'name':name, 'shadow price':c.pi, 'slack': c.slack}
             for name, c in prblToGo.constraints.items()]
        print(pd.DataFrame(o))
        Schattenpreise = pd.DataFrame(o)
    ...
    #print(pulp.LpMinimize)
    # # für die Maximierung
    # # milp.sense = pulp.LpMaximize

    # # für die Minimierung
    # milp.sense = pulp.LpMinimize

    # # # Zielfunktion dem Modell hinzufügen
    # # milp.setObjective(objectiveTotalCost)
    ...
    # milp.solve()
    # #schreibe Status
    # print(pulp.LpStatus[milp.status])

    # # schreibe den Zielfunktionswert
    # print(pulp.value(milp.objective))


    #def print_write_Results(solvedProblem):
            ...
            for i in transportA:
                ...
                for j in newTransportB:
                    for p in allProductsBom:
                        ...
                        # nur wenn die Liefermenge > 0, wird dict zur transportOutput
                        # Liste hinzugefügt
    ...
    print(get_Kosten_Output())

    pathName = 'Ergebnisse/'
    excelWorkbookName = (
        pd.Timestamp.now().strftime(format = "%Y%m%d_%H_%M_%S")
        + '_Ergebnis.xlsx'
        )

    def writeSolutionToExcel(path = pathName, workbookName = excelWorkbookName):
        ...
        Schattenpreise.to_excel(writer, index=True, sheet_name="new")
    ...
    excel_file_result_name = writeSolutionToExcel()
    return excel_file_result_name