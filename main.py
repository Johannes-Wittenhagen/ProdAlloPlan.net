...
import PAPModel_Sensitivity_Relocation2
...
def graph():
    kostenanteile = np.random.normal(20000, 25000, 5000)
    plt.hist(kostenanteile, 50)
    plt.show()
...
def poptana(model, excel_sensitivity_file_name):
    ...
    if model == "Relocation":
        poptana_result_file_name = PAPModel_Sensitivity_Relocation2.relocation(excel_file_name,excel_sensitivity_file_name, optimize_cost, optimize_cust, optimize_extra)
    if model == "Demand":
...
#Anzeigen eines loading GIFs
def please_wait(gifnr):
    loading_window = Toplevel()
    loading_window.title("Ergebnisse des Modells")
    loading_window.iconbitmap("C:/GUI/PycharmProjects/ProdAlloPlan.net/images/BVL123_RZ_Logo_rgb.ico")
    ...
    while counter < gifnr:
        ...
        photo = ImageTk.PhotoImage(file="images/spinner-5.gif", format="gif -index " + str(counter))
