...
def transport(name, name2, opt_1, opt_2, opt_3):
    ...
    transportCon2 = transportCon.copy()
    ...
    transport_sensitivity=pd.read_excel(excel_file_name2, sheet_name="Transportverbindung",
                             index_col=[0,1,2], header=6, usecols='B:E')
    ...
    productionOutput = []
    i = 0
    while i < (transport_sensitivity.reset_index()['Durchgang'].max()+1):

        if i == 0:
            pass
        else:
            Transport = transportCon2.copy()
            Transport2= Transport.reset_index()
            Transport_Index = transport_sensitivity.loc[[i]].reset_index().set_index(['Von','Nach']).drop(['Durchgang'], axis =1).sort_values(['Von'])


            j =0
            while j < Transport.shape[0]:
                Transport.iloc[j,0] = Transport.iloc[j,0]*(1+Transport_Index.loc[(Transport2.iloc[j,1],Transport2.iloc[j,2]), 'Anpassung'])
                j +=1
            transportCon = Transport
    ...
    table.to_excel(writer, index=True, sheet_name='Zusammenfassung')
