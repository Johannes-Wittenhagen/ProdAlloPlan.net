...
def hour(name, name2, opt_1, opt_2, opt_3):
    ...
    ressources_sensitivity=pd.read_excel(excel_file_name2, sheet_name="Ressourcen",
                             index_col=[0,1,], header=6, usecols='B:P')
    ...
    productionOutput = []
    i=0
    #ressources_sensitivity.shape[0]+1
    while i < (ressources_sensitivity.reset_index()['Durchgang'].max()+1):

        if i == 0:
            pass
        else:
            ID_Werk_Index = ressources_sensitivity.loc[[i]].reset_index().set_index('ID_Werk').drop(['Durchgang'], axis =1)
            ID_Werk_oI = ID_Werk_Index.reset_index()
            print(ID_Werk_oI.iloc[0,0])
            for j in range(0,ID_Werk_Index.shape[0]):
                #print(i)

             ressources.loc[ID_Werk_oI.iloc[j,0], 'r_Stundensatz']= ID_Werk_Index.loc[ID_Werk_oI.iloc[j,0], 'r_Stundensatz']
