...
def demand(name, name2, opt_1, opt_2, opt_3):
    global opt1
    opt1 = opt_1
    global opt2
    opt2 = opt_2
    ...
    productionOutput = []
    i = 0
    while i < (nachfrage_sensitivity.reset_index()['Durchgang'].max()+1):

        if i == 0:
            pass
        else:
            Nachfrage = demand2.copy()
            Nachfrage_Index = nachfrage_sensitivity.loc[[i]].reset_index().set_index('Kundenregion').drop(['Durchgang'], axis =1).sort_values(['Kundenregion'])
            Nachfrage_oI = Nachfrage_Index.reset_index()
            Nachfrage.reset_index(inplace = True)
            for j in range(0,Nachfrage_Index.shape[0]):

                Nachfrage.loc[Nachfrage['ID_Kundenregion'] == Nachfrage_oI.iloc[j,0],'Nachfrage'] = round((1+Nachfrage_Index.iloc[j,0])*Nachfrage.loc[Nachfrage['ID_Kundenregion'] == Nachfrage_oI.iloc[j,0],'Nachfrage'])

            Nachfrage.set_index(['Perioden','ID_Kundenregion','ID_Produktzustand'],inplace=(True))
            demand = Nachfrage
            del Nachfrage
